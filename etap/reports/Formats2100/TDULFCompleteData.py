...
import collections
import openpyxl
from etap.reports.Formats2100 import TDULFReportsCommons as comms
...
def load_events_data(wb, ws, cursor, table_names_list):
    ...
    millisec = comms.is_milli_sec(cursor)
    ...
    for i in range(0, total_rows_in_db):
        ...
        if time_format[0][0] == 0 or time_format[0][0] == 1 or time_format[0][0] == 2:
            if millisec:
                ws[coordinates_for_Time] = sql_all_rows_data[i][column_time].split(None, 1)[1]
            else:
                ws[coordinates_for_Time] = sql_all_rows_data[i][column_time].split(None, 1)[1][:-4]
        else:
            if millisec:
                ws[coordinates_for_Time] = sql_all_rows_data[i][column_time][12:]
            else:
                ws[coordinates_for_Time] = sql_all_rows_data[i][column_time][12:19]
        ...
        if time_format[0][0] == 0 or time_format[0][0] == 1 or time_format[0][0] == 2:
            ws[coordinates_for_Date] = sql_all_rows_data[i][column_time].split(None, 1)[0]
        else:
            ws[coordinates_for_Date] = sql_all_rows_data[i][column_time][:11]
        # ws[coordinates_for_Date] = sql_all_rows_data[i][column_time].split(None, 1)[0]
...
def load_device_profile_data(wb, ws, cursor, table_names_list):
    ...
    for i in range(0, total_rows_in_db):
        ...
        energize_date_final = comms.read_time_format(cursor, energize_date_value, 1).date()
...
def load_alert_data(wb, ws, cursor, table_names_list):
    ...
    for i in range(0, total_rows_in_db):
        ...
        time_stamp = resultID_to_timestamp_dict[sql_all_rows_data[i][column_result_id]][0]
        # date_value = time_stamp[:time_stamp.find(' ')]
        # time_value = time_stamp[time_stamp.find(' ')+1:]
        date_value, time_value = comms.date_and_time_from_timeStamp(cursor, time_stamp)

        ws[coordinates_for_time]._style = copy(second_row_style["Alerts_Time"])
        if millisec:
            ws[coordinates_for_time] = time_value
        else:
            ws[coordinates_for_time] = time_value[:-4]
...
def load_results_data(wb, ws, cursor, table_names_list):
    ...
    command = '''
                select 
                    timeid, time, totalloadmwpha, totalloadmwphb, totalloadmwphc, 
                    totalloadmvarpha, totalloadmvarphb, totalloadmvarphc,
                    totalsourcemwpha, totalsourcemwphb, totalsourcemwphc,
                    totalsourcemvarpha, totalsourcemvarphb, totalsourcemvarphc,
                    mwlosspha, mwlossphb, mwlossphc, mvarlosspha, mvarlossphb, mvarlossphc,
                    totaldcload, totaldcsource, windgeneration, solargeneration
                from tdtimeid left join tdsysresult using ("resultid")
              '''
    cursor.execute(command)
    sql_all_rows_data = cursor.fetchall()
    millisec = comms.is_milli_sec(cursor)
    ...
    for i in range(0, total_rows_in_db):
        # Convert time and date to saperate columns...
        time_stamp = sql_all_rows_data[i][1]
        date_value, time_value = comms.date_and_time_from_timeStamp(cursor, time_stamp)
        # date_value = time_stamp[:time_stamp.find(' ')]
        # time_value = time_stamp[time_stamp.find(' ')+1:]
        ...
        if sql_all_rows_data[i][2] is not None:
        ...
        if sql_all_rows_data[i][5] is not None:
        ...
        if sql_all_rows_data[i][8] is not None:
        ...
        if sql_all_rows_data[i][11] is not None:
        ...
        if sql_all_rows_data[i][20] is not None:
            total_dc_load = (sql_all_rows_data[i][20]*1000)
        else:
            total_dc_load = ''
        ...
        if sql_all_rows_data[i][21] is not None:
            total_dc_source = (sql_all_rows_data[i][21]*1000)
        else:
            total_dc_source = ''
        ...
        if total_ac_source_kw is not None and total_ac_source_kw != '':
        ...
        if sql_all_rows_data[i][22] is not None:
            ws[coordinates_for_windkw] = (sql_all_rows_data[i][22]*1000)
        else:
            ws[coordinates_for_windkw] = ''
        ...
        if sql_all_rows_data[i][23] is not None:
            ws[coordinates_for_solarkw] = (sql_all_rows_data[i][23]*1000)
        else:
            ws[coordinates_for_solarkw] = ''
        ...
        if sql_all_rows_data[i][15] > 0 or sql_all_rows_data[i][11] > 0:
            ...
            if sql_all_rows_data[i][11] > 0:
                ...
                if millisec:
                    ...
                    ws[coordinates_for_maxVoltTime] = time_value[:-4]
            ...
            if sql_all_rows_data[i][15] > 0:
                ...
                if millisec:
                    ...
                    ws[coordinates_for_minVoltTime] = time_value[:-4]
...
def load_summary_group_data(wb, ws, cursor, table_names_list):
    ...
    end_time_stamp = comms.read_time_format(cursor, study_case_info_data[0][2])
    ...
    for group in range(0, number_of_groups):
        ...
        for i in range(0, len(data_for_current_group)):
            ...
            time_stamp_for_i = comms.read_time_format(cursor, resultID_to_timestamp_dict[data_for_current_group[i][1]][0])

            if(i == len(data_for_current_group)-1):
                if(end_time_stamp != comms.read_time_format(cursor, resultID_to_timestamp_dict[data_for_current_group[i][1]][0])):
                    time_stamp_for_i1 = end_time_stamp
                else:
                    time_stamp_for_i1 = comms.read_time_format(cursor, resultID_to_timestamp_dict[data_for_current_group[i][1]][0])
            else:
                time_stamp_for_i1 = comms.read_time_format(cursor, resultID_to_timestamp_dict[data_for_current_group[i+1][1]][0])
        ...
        time_stamp_for_maxWindGen = wind_gen_to_time_stamp[max(
            list_of_wind_gen)]
        ws[coordinates_for_maxWindGen] = max(list_of_wind_gen) * 1000
        ws[coordinates_for_maxWindGen_date] = time_stamp_for_maxWindGen.strftime(
            '%m-%d-%Y')
        if millisec:
            ws[coordinates_for_maxWindGen_time] = time_stamp_for_maxWindGen.time().strftime('%X') + '.' + time_stamp_for_maxWindGen.time().strftime('%f')[:3]
        else:
        ...
        time_stamp_for_maxSolarGen = solar_gen_to_time_stamp[max(
            list_of_solar_gen)]
        ws[coordinates_for_maxSolarGen] = max(list_of_solar_gen) * 1000
        ws[coordinates_for_maxSolarGen_date] = time_stamp_for_maxSolarGen.strftime(
            '%m-%d-%Y')
        if millisec:
            ws[coordinates_for_maxSolarGen_time] = time_stamp_for_maxSolarGen.time().strftime('%X') + '.' + time_stamp_for_maxSolarGen.time().strftime('%f')[:3]
        else:
        ...
        time_stamp_for_maxTotalACGen = AC_total_gen_to_time_stamp[max(
            list_of_AC_total_gen)]
        ws[coordinates_for_maxTotalACGen] = max(list_of_AC_total_gen) * 1000
        ws[coordinates_for_maxTotalACGen_date] = time_stamp_for_maxTotalACGen.strftime(
            '%m-%d-%Y')
        if millisec:
            ws[coordinates_for_maxTotalACGen_time] = time_stamp_for_maxTotalACGen.time().strftime('%X') + '.' + time_stamp_for_maxTotalACGen.time().strftime('%f')[:3]
        else:
        ...
        time_stamp_for_maxTotalDCGen = DC_total_gen_to_time_stamp[max(
            list_of_DC_total_gen)]
        ws[coordinates_for_maxTotalDCGen] = max(list_of_DC_total_gen) * 1000
        ws[coordinates_for_maxTotalDCGen_date] = time_stamp_for_maxTotalDCGen.strftime(
            '%m-%d-%Y')
        if millisec:
            ws[coordinates_for_maxTotalDCGen_time] = time_stamp_for_maxTotalDCGen.time().strftime('%X') + '.' + time_stamp_for_maxTotalDCGen.time().strftime('%f')[:3]
        else:
        ...
        time_stamp_for_maxTotalACLoad = AC_total_load_to_time_stamp[max(
            list_of_AC_total_load)]
        ws[coordinates_for_maxTotalACLoad] = max(list_of_AC_total_load) * 1000
        ws[coordinates_for_maxTotalACLoad_date] = time_stamp_for_maxTotalACLoad.strftime(
            '%m-%d-%Y')
        if millisec:
            ws[coordinates_for_maxTotalACLoad_time] = time_stamp_for_maxTotalACLoad.time().strftime('%X') + '.' + time_stamp_for_maxTotalACLoad.time().strftime('%f')[:3]
        else:
        ...
        time_stamp_for_maxTotalDCLoad = DC_total_load_to_time_stamp[max(
            list_of_DC_total_load)]
        ws[coordinates_for_maxTotalDCLoad] = max(list_of_DC_total_load) * 1000
        ws[coordinates_for_maxTotalDCLoad_date] = time_stamp_for_maxTotalDCLoad.strftime(
            '%m-%d-%Y')
        if millisec:
            ws[coordinates_for_maxTotalDCLoad_time] = time_stamp_for_maxTotalDCLoad.time().strftime('%X') + '.' + time_stamp_for_maxTotalDCLoad.time().strftime('%f')[:3]
        else:
...
def load_system_summary_data(wb, ws, cursor, table_names_list):
    ...
    for i in range(0, total_rows_system_result):
        time_stamp_for_i = comms.read_time_format(cursor, resultID_to_timestamp_dict[system_result_data[i][1]][0])

        if(i == total_rows_system_result-1):
            if end_time_stamp != comms.read_time_format(cursor, resultID_to_timestamp_dict[system_result_data[i][1]][0]):
                time_stamp_for_i1 = end_time_stamp
            else:
                time_stamp_for_i1 = comms.read_time_format(cursor, resultID_to_timestamp_dict[system_result_data[i][1]][0])
        else:
            time_stamp_for_i1 = comms.read_time_format(cursor, resultID_to_timestamp_dict[system_result_data[i+1][1]][0])
    ...
    command_max_vpha = '''
                        ...
                        inner join (
                            SELECT
                                "time", timeid, resultid, busiid, vpha
                            from
                                TDTimeID
                            inner join (
                                Select
                                    resultid, busiid, vpha
                                from
                                    tdbusresult
                                where
                                    vpha = (
                        ...
                        using (busiid)
                        '''
    cursor.execute(command_max_vpha)
    max_v_pha_data = cursor.fetchall()
    ...
    #     Va_max_list)][1]
    ws[coordinates_for_maxVaBusID] = max_v_pha_data[0][1]
    ...
    ws[coordinates_for_maxVaMag] = max_v_pha_data[0][2]

    # time_stamp_maxVa = resultID_to_timestamp_dict[Va_max_to_resultID_busID[max(Va_max_list)][0]][0]
    time_stamp_maxVa = max_v_pha_data[0][0]
    date_value_maxVaDate, time_value_maxVaTime = comms.date_and_time_from_timeStamp(cursor, time_stamp_maxVa)
    ...
    if millisec:
        ...
        ws[coordinates_for_maxVaTime] = time_value_maxVaTime[:-4]

    # Max PhaseB Voltage section
    command_max_vphb = command_max_vpha.replace('vpha', 'vphb')
    cursor.execute(command_max_vphb)
    max_v_phb_data = cursor.fetchall()
    ...
    #     Vb_max_list)][1]
    ws[coordinates_for_maxVbBusID] = max_v_phb_data[0][1]
    ...
    ws[coordinates_for_maxVbMag] = max_v_phb_data[0][2]
    # time_stamp_maxVb = resultID_to_timestamp_dict[Vb_max_to_resultID_busID[max(Vb_max_list)][0]][0]
    time_stamp_maxVb = max_v_phb_data[0][0]
    date_value_maxVbDate, time_value_maxVbTime = comms.date_and_time_from_timeStamp(cursor, time_stamp_maxVb)
    ...
    if millisec:
        ...
        ws[coordinates_for_maxVbTime] = time_value_maxVbTime[:-4]

    # Max PhaseC Voltage section
    command_max_vphc = command_max_vpha.replace('vpha', 'vphc')
    cursor.execute(command_max_vphc)
    max_v_phc_data = cursor.fetchall()
    ...
    #     Vc_max_list)][1]
    ws[coordinates_for_maxVcBusID] = max_v_phc_data[0][1]
    ...
    ws[coordinates_for_maxVcMag] = max_v_phc_data[0][2]
    # time_stamp_maxVc = resultID_to_timestamp_dict[Vc_max_to_resultID_busID[max(Vc_max_list)][0]][0]
    time_stamp_maxVc = max_v_phc_data[0][0]
    date_value_maxVcDate, time_value_maxVcTime = comms.date_and_time_from_timeStamp(cursor, time_stamp_maxVc)
    ...
    if millisec:
        ...
        ws[coordinates_for_maxVcTime] = time_value_maxVcTime[:-4]

    # Writing data for "Minimum Voltage" fields in excel
    # Min PhaseA Voltage section
    command_min_vpha = command_max_vpha.replace('max', 'min')
    cursor.execute(command_min_vpha)
    min_v_pha_data = cursor.fetchall()
    ...
    #     Va_min_list)][1]
    ws[coordinates_for_minVaBusID] = min_v_pha_data[0][1]
    ...
    ws[coordinates_for_minVaMag] = min_v_pha_data[0][2]
    # time_stamp_minVa = resultID_to_timestamp_dict[Va_min_to_resultID_busID[min(Va_min_list)][0]][0]
    time_stamp_minVa = min_v_pha_data[0][0]
    date_value_minVaDate, time_value_minVaTime = comms.date_and_time_from_timeStamp(cursor, time_stamp_minVa)
    ...
    if millisec:
        ...
        ws[coordinates_for_minVaTime] = time_value_minVaTime[:-4]

    # Min PhaseB Voltage section
    command_min_vphb = command_min_vpha.replace('vpha', 'vphb')
    cursor.execute(command_min_vphb)
    min_v_phb_data = cursor.fetchall()
    ...
    #     Vb_min_list)][1]
    ws[coordinates_for_minVbBusID] = min_v_phb_data[0][1]
    ...
    ws[coordinates_for_minVbMag] = min_v_phb_data[0][2]
    # time_stamp_minVb = resultID_to_timestamp_dict[Vb_min_to_resultID_busID[min(Vb_min_list)][0]][0]
    time_stamp_minVb = min_v_phb_data[0][0]
    date_value_minVbDate, time_value_minVbTime = comms.date_and_time_from_timeStamp(cursor, time_stamp_minVb)
    ...
    if millisec:
        ...
        ws[coordinates_for_minVbTime] = time_value_minVbTime[:-4]

    # Min PhaseC Voltage section
    command_min_vphc = command_min_vpha.replace('vpha', 'vphc')
    cursor.execute(command_min_vphc)
    min_v_phc_data = cursor.fetchall()
    ...
    #     Vc_min_list)][1]
    ws[coordinates_for_minVcBusID] = min_v_phc_data[0][1]
    ...
    ws[coordinates_for_minVcMag] = min_v_phc_data[0][2]
    # time_stamp_minVc = resultID_to_timestamp_dict[Vc_min_to_resultID_busID[min(Vc_min_list)][0]][0]
    time_stamp_minVc =  min_v_phc_data[0][0]
    date_value_minVcDate, time_value_minVcTime = comms.date_and_time_from_timeStamp(cursor, time_stamp_minVc)
    ...
    if millisec:
        ...
        ws[coordinates_for_minVcTime] = time_value_minVcTime[:-4]
...
def load_cover_data(wb, ws, cursor, table_names_list, templatePath):
    ...
    ws[coordinates_for_OutputFileName] = study_case_info_data[0][32]

    coordinates_for_config = excel_column_name_to_coordinates_info["Cover_config"]
    ws[coordinates_for_config] = study_case_info_data[0][33]

    coordinates_for_revision = excel_column_name_to_coordinates_info["Cover_Revision"]
    ws[coordinates_for_revision] = study_case_info_data[0][34]
...
def load_study_info_data(wb, ws, cursor, table_names_list):
    ...
    simulation_start_date, simulation_start_time = comms.date_and_time_from_timeStamp(cursor, simulation_start_time_stamp)
    ...
    simulation_stop_date, simulation_stop_time = comms.date_and_time_from_timeStamp(cursor, simulation_stop_time_stamp)
    ...
    if millisec:
        ...
        ws[coordinates_for_simStartTime] = simulation_start_time[:-4]
    ...
    coordinates_for_simStopTime = comms.increase_column_by_one(coordinates_for_simStopDate)
    if millisec:
        ws[coordinates_for_simStopTime] = simulation_stop_time
    else:
        ws[coordinates_for_simStopTime] = simulation_stop_time[:-4]
    ...
    ws[coordinates_for_externalData] = comms.is_tolerance_applied(
        study_case_info_data[0][41])

    wind_external_data_raw = study_case_info_data[0][42]
    ...
    pv_external_data_raw = study_case_info_data[0][43]
    ...
    ws[coordinates_for_annualLoadGrowth] = study_case_info_data[0][44]
    ...
    ws[coordinates_for_xmerZTolPercent] = study_case_info_data[0][47]
    ...
    ws[coordinates_for_xmerXTolPercent] = study_case_info_data[0][65]
    ...
    ws[coordinates_for_oLHResistancePercent] = study_case_info_data[0][56]
    ...
    ws[coordinates_for_lineLengthPercent] = study_case_info_data[0][50]
    ...
    ws[coordinates_for_cableLengthPercent] = study_case_info_data[0][53]
    ...
    ws[coordinates_for_lineResistancePercent] = study_case_info_data[0][59]
    ...
    ws[coordinates_for_cableResistancePercent] = study_case_info_data[0][62]
    ...
    calcStart_date_value, calcStart_time_value = comms.date_and_time_from_timeStamp(cursor, calcStart_time_stamp)
    ...
    calcStop_date_value, calcStop_time_value = comms.date_and_time_from_timeStamp(cursor, calcStop_time_stamp)
    ...
    ws[coordinates_for_calcStartTime] = calcStart_time_value[:-4]
    ...
    ws[coordinates_for_calcStopTime] = calcStop_time_value[:-4]
    ...
    coordinates_for_quasi = comms.get_updated_columns_coordinates(
        excel_column_name_to_coordinates_info["Info_Quasi"], 7, -4)
    if study_case_info_data[0][70] == 0:
        ws[coordinates_for_quasi] = 'No'
    else:
        ws[coordinates_for_quasi] = 'Yes'

    coordinates_for_millisec = comms.get_updated_columns_coordinates(
        excel_column_name_to_coordinates_info["Info_MilliSecond"], 8, -4)
    if study_case_info_data[0][71] == 0:
        ws[coordinates_for_millisec] = 'No'
    else:
        ws[coordinates_for_millisec] = 'Yes'

    coordinates_for_interpolate = comms.get_updated_columns_coordinates(
        excel_column_name_to_coordinates_info["Info_Interpolate"], 9, -4)
    if study_case_info_data[0][72] == 0:
        ws[coordinates_for_interpolate] = 'No'
    else:
        ws[coordinates_for_interpolate] = 'Yes'


def load_energy_storage_data(wb, ws, cursor, table_names_list):
    command = '''select tdtimeid.timeid, tdtimeid.time, tdenergystorage.deviceid, tdenergystorage.termvp, 
    tdenergystorage.ampflowp, tdenergystorage.powerflowp, tdenergystorage.energystored, tdenergystorage.soc
